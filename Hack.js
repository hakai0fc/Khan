...
/* Element(s?) */
const splashScreen = document.createElement('div'); // Use div ao invés de tag inexistente
...
if(document.querySelector("link[rel~='icon']")) document.querySelector("link[rel~='icon']").href = 'https://r2.e-z.host/4d0a0bea-60f8-44d6-9e74-3032a64a9f32/ukh0rq22.png';
...
new MutationObserver((mutationsList)=>{ for(let m of mutationsList) if(m.type==='childList') plppdo.emit('domChanged'); }).observe(document.body,{childList:true,subtree:true});
...
function sendToast(text,duration=5000,gravity='bottom'){ Toastify({text,duration,gravity,position:"center",stopOnFocus:true,style:{background:"#000"}}).showToast(); }
...
async function loadCss(url){ return new Promise(r=>{ const link=document.createElement('link'); link.rel='stylesheet'; link.type='text/css'; link.href=url; link.onload=()=>r(); document.head.appendChild(link); }); }
