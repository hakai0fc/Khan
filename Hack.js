...
if (favicon) favicon.href = 'https://r2.e-z.host/4d0a0bea-60f8-44d6-9e74-3032a64a9f32/ukh0rq22.png';
...
async function showSplashScreen() { 
    ...
    splashScreen.innerHTML = '<span style="color:#72ff72;">HAKAI</span><span style="color:white;">.Khan</span>';
...
async function loadScript(url, label) { 
    return fetch(url).then(response => response.text()).then(script => { loadedPlugins.push(label); eval(script); }).catch(e=>{console.error("Erro ao carregar script:",url,e);});
}
async function loadCss(url) { 
    return new Promise((resolve) => { const link = document.createElement('link'); link.rel = 'stylesheet'; link.type = 'text/css'; link.href = url; link.onload = () => resolve(); document.head.appendChild(link); });
}
